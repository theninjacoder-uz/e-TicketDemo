...
public interface ScheduleRepository extends JpaRepository<ScheduleEntity, Long> {
    ...
    @Query(value = "select fsc.train_id as train_id, fsc.departure_time as departure_time, ssc.arrival_time as arrival_time from schedule fsc inner join schedule ssc on fsc.train_id = ssc.train_id where fsc.travel_date>=?1 and fsc.travel_date < ?2 and fsc.station_id = ?3 and ssc.station_id = ?4", nativeQuery = true)
